...
namespace AutoCompare.Configuration
{
	...
	internal class ComparerConfiguration<T> : ComparerConfiguration, IComparerConfiguration<T>, IPrecompile where T : class
	{
		...
		public IComparerConfiguration<T> IgnoreType<TType>()
		{
			IgnoreType(typeof(TType));
			return this;
		}

		public IComparerConfiguration<T> IgnoreType(Type typeToIgnore)
		{
			if (!this.IgnoredTypes.Contains(typeToIgnore))
